<?php

namespace Tests\Feature\Controllers\Cms;

use App\Models\CmssectionModel;
use Tests\Support\ApiTestCase;
use Tests\Support\Traits\AuthTestTrait;

/**
 * @internal
 */
final class CmssectionControllerTest extends ApiTestCase
{
    use AuthTestTrait;

    private CmssectionModel $model;

    protected function setUp(): void
    {
        parent::setUp();
        $this->model = new CmssectionModel();
    }

    public function testEndpointsRequireAuthentication(): void
    {
        $result = $this->get('/api/v1/cms/cmssections');
        $result->assertStatus(401);
    }

    public function testEndpointsRequireAdminRole(): void
    {
        $this->actAs('user');

        $list = $this->get('/api/v1/cms/cmssections');
        $list->assertStatus(403);
    }

    public function testAdminCrudFlowWorks(): void
    {
        $this->actAs('admin');

        $create = $this->withBodyFormat('json')->post('/api/v1/cms/cmssections', [
            'name' => 'Tech Blog',
            'slug' => 'tech-blog',
            'description' => 'A blog about tech',
            'status' => 'active',
        ]);
        $create->assertStatus(201);
        $createJson = $this->getResponseJson($create);
        $id = (int) ($createJson['data']['id'] ?? 0);
        $this->assertGreaterThan(0, $id);

        $list = $this->get('/api/v1/cms/cmssections');
        $list->assertStatus(200);

        $show = $this->get("/api/v1/cms/cmssections/{$id}");
        $show->assertStatus(200);

        $update = $this->withBodyFormat('json')->put("/api/v1/cms/cmssections/{$id}", [
            'status' => 'inactive',
        ]);
        $update->assertStatus(200);

        $delete = $this->delete("/api/v1/cms/cmssections/{$id}");
        $delete->assertStatus(200);

        $this->assertNull($this->model->find($id));
        $this->assertNotNull($this->model->withDeleted()->find($id));
    }
}
